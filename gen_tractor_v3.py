...
const t_ui = {
    en: {
        ...
        groupBooking: 'Small Land Group Booking (-20%)', groupHelp: 'Book with nearest neighbors to save fuel costs',
        ...
        loc_fixed: '📍 Fixed Location Tracking Active'
    },
    or: {
        ...
        loc_fixed: '📍 ସ୍ଥିର ସ୍ଥାନ ଟ୍ରାକିଂ ସକ୍ରିୟ'
    }
};
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    try {
                        ...
                        setVillage(placeName);
                        setLocation(fullLoc);
                        setLocating(false);
                        setStage('category');
                        toast.success(`${l.loc_fixed}: ${fullLoc}`);
                    } catch (err) {
                        // Fallback but keep it "sticky"
                        setVillage('Patia'); setLocation('Patia, Bhubaneswar');
                        setLocating(false); setStage('category');
                        toast.success('📍 Patia, Bhubaneswar');
                    }
                },
                (err) => {
                    setLocating(false);
                    toast.error(lang === 'or' ? 'GPS ମିଳିଲା ନାହିଁ, ହାତରେ ଲେଖନ୍ତୁ' : 'GPS error. Entering demo mode: Patia');
                    // Demo fallback if GPS denied
                    setVillage('Patia'); setLocation('Patia, Bhubaneswar');
                    setStage('category');
                }
            );
                    ...
                    <div className="w-full max-w-md bg-white dark:bg-slate-900 p-5 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div className="ml-auto flex items-center gap-1 bg-green-100 dark:bg-green-900/30 px-3 py-1 rounded-full">
                                ...
                                <span className="text-[9px] font-black text-green-700 dark:text-green-400">Stable Link</span>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSearch} className="space-y-3">
                            ...
                            <button type="button" onClick={()=>setIsGroup(!isGroup)}
                                ...
                                <div className={`w-9 h-5 rounded-full p-1 transition-colors ${isGroup?'bg-green-500':'bg-slate-300 dark:bg-slate-600'}`}>
                                    <div className={`w-3 h-3 bg-white rounded-full transition-all ${isGroup?'translate-x-4':'translate-x-0'}`}/>
                                </div>
                            </button>

                            <div className="flex gap-2">
                                <button type="button" onClick={handleGPS} disabled={locating}
                                    className="flex-1 py-3 bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition-colors">
